...
export class ManageOrderComponent implements OnInit {
  ...
  getCategorys() {
    ...
    this.categoryService.getCategory(dataOfcat).subscribe(
      (response: any) => {
  ...
  add(){
    ...
    if(productName === undefined){
      ...
      this.dataSource.push({id:formData.product.id,name:formData.product.name , 
        category: formData.category.name , quantity : formData.quantity , 
        price : formData.price , total : formData.total});
        this.dataSource = [...this.dataSource];
        this.snackbarService.success(GlobalConstants.productAdded)
    }
    else{
      this.snackbarService.error(GlobalConstants.productExist)
    }
  }
